...
const AdminMiniGameTab: React.FC = () => {
    ...
    const handleInitGame = () => {
        ...
        socketRef.current.emit('ADMIN_MINIGAME_ACTION', payload);
    ...
    const handleStartShuffle = () => {
        ...
        socketRef.current.emit('ADMIN_MINIGAME_ACTION', { type: 'START_SHUFFLE' });
            ...
            <Tabs>
                <Tabs.Tab title='紅包' key='red-envelope'>
                    <Space direction='vertical' block>
                        ...
                        <Card title='安慰獎設定' style={{ borderColor: '#f0a020' }}>
                            <Space direction='vertical' block>
                                <Input
                                    ...
                                    onChange={(val) => {
                                        setConsolation((prev) => ({ ...prev, name: val ?? '' }));
                                        localStorage.setItem(
                                            'miniGameConsolation',
                                            JSON.stringify({ ...consolation, name: val ?? '' })
                                        );
                                    }}
                                    placeholder='安慰獎名稱'
                                />
                        ...
                        <Button color='warning' onClick={handleStartShuffle} disabled={!isRedEnvelope || !isIdlePhase}>
                            開始洗牌
                        </Button>
